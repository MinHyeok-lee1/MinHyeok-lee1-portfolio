...
사이드바에 표시되는 페이지의 제모고가 순서는 `_meta.json`파일에서 키-값쌍으로 구성해야 한다.  
...
최상위 \_meta.json파일에는 최상위 페이지 및 폴더에 대한 메타 정보가 포함되어 있다.
...
그러면 Nextra는 \_meta.json의 fruits 키가 인덱스 페이지가 있는 폴더를 정의한다는 것을 알게 된다.  
...
\_meta.json에 href가 있는 항목을 추가하여 사이드바에 외부 링크를 추가할 수 있습니다:
...
항상 새 탭에서 링크를 열려면 아래와 같이 "newWindow": true 옵션을 활성화합니다.
...
<Callout type="default">
  sidebar.titleComponent 테마 옵션과 함께 사이드바에서 제목 및 구분선의 모양을
  사용자 지정할 수 있다.
</Callout>
