...
class Foo<T extends Object> {
  // Foo 에서 T에 제공된 모든 유형은 null을 허용하지 않아야 한다.
}
```
...
유형 별칭(typedef 키워드로 선언되기 때문에 종종 typedef라고 함)은 유형을 참조하는 간결한 방법이다. 다음은 IntList라는 유형 별칭을 선언하고 사용하는 예이다.
...
대부분의 상황에서 함수에 대해 typedef 대신 인라인 함수 유형을 사용하는 것이 좋습니다. 그러나 함수 typedef는 여전히 유용할 수 있다.
